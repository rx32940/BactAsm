rule fastq_dump:    
    ...
    shell:
        """
        fastq-dump {params.format} {params.pe} -O {params.output_dir}/SRA {params.sra}
        mv {params.output_dir}/SRA/{params.sra}_1.fastq.gz {output_dir}SRA/{wildcards.sample}_1.fastq.gz
        mv {params.output_dir}/SRA/{params.sra}_2.fastq.gz {output_dir}SRA/{wildcards.sample}_2.fastq.gz
        """
...
rule multiqc:
    ...
    output_dir +"SRA_fastqc/{sample}_2_fastqc.zip"], sample=config["samples"])
    output:
        "{params.output_dir}multiqc_report.html"
    conda:
        "../env/multiqc.yaml"
    params:
        output_dir=output_dir
